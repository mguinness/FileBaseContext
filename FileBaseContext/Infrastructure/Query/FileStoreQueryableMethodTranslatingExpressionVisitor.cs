...
public class FileBaseContextQueryableMethodTranslatingExpressionVisitor : QueryableMethodTranslatingExpressionVisitor
{
    private readonly FileBaseContextExpressionTranslatingExpressionVisitor _expressionTranslator;
    private readonly IModel _model;
    private readonly FileBaseContextProjectionBindingExpressionVisitor _projectionBindingExpressionVisitor;
    private readonly SharedTypeEntityExpandingExpressionVisitor _weakEntityExpandingExpressionVisitor;

    public FileBaseContextQueryableMethodTranslatingExpressionVisitor(
        QueryableMethodTranslatingExpressionVisitorDependencies dependencies,
        QueryCompilationContext queryCompilationContext)
        : base(dependencies, queryCompilationContext, subquery: false)
    {
        _expressionTranslator = new FileBaseContextExpressionTranslatingExpressionVisitor(queryCompilationContext, this);
        _weakEntityExpandingExpressionVisitor = new SharedTypeEntityExpandingExpressionVisitor(_expressionTranslator);
        _projectionBindingExpressionVisitor = new FileBaseContextProjectionBindingExpressionVisitor(this, _expressionTranslator);
        _model = queryCompilationContext.Model;
    }

    protected FileBaseContextQueryableMethodTranslatingExpressionVisitor(
        FileBaseContextQueryableMethodTranslatingExpressionVisitor parentVisitor)
        : base(parentVisitor.Dependencies, parentVisitor.QueryCompilationContext, subquery: true)
    {
        _expressionTranslator = new FileBaseContextExpressionTranslatingExpressionVisitor(QueryCompilationContext, parentVisitor);
        _weakEntityExpandingExpressionVisitor = new SharedTypeEntityExpandingExpressionVisitor(_expressionTranslator);
        _projectionBindingExpressionVisitor = new FileBaseContextProjectionBindingExpressionVisitor(this, _expressionTranslator);
    ...
    protected override QueryableMethodTranslatingExpressionVisitor CreateSubqueryVisitor()
        => new FileBaseContextQueryableMethodTranslatingExpressionVisitor(this);
    ...
    protected override ShapedQueryExpression TranslateDefaultIfEmpty(ShapedQueryExpression source, Expression defaultValue)
    {
        ...
        {
            ((FileBaseContextQueryExpression)source.QueryExpression).ApplyDefaultIfEmpty();
    ...
    {
        ((FileBaseContextQueryExpression)source.QueryExpression).ApplyDistinct();
        ...
        LambdaExpression resultSelector)
    {
        ...
        var outerShaperExpression = ((FileBaseContextQueryExpression)outer.QueryExpression).AddInnerJoin(
            (FileBaseContextQueryExpression)inner.QueryExpression,
            outerKeySelector,
            innerKeySelector,
        ...
        LambdaExpression resultSelector)
    {
        ...
        var outerShaperExpression = ((FileBaseContextQueryExpression)outer.QueryExpression).AddLeftJoin(
            (FileBaseContextQueryExpression)inner.QueryExpression,
            outerKeySelector,
            innerKeySelector,
            outer.ShaperExpression,
    ...
    protected override ShapedQueryExpression TranslateSelect(ShapedQueryExpression source, LambdaExpression selector)
    {
        ...
        var queryExpression = (FileBaseContextQueryExpression)source.QueryExpression;
        ...
        LambdaExpression resultSelector)
    {
        ...
        {
            var outerShaperExpression = ((FileBaseContextQueryExpression)source.QueryExpression).AddSelectMany(
                (FileBaseContextQueryExpression)inner.QueryExpression, source.ShaperExpression, inner.ShaperExpression, defaultIfEmpty);
    ...
    protected override Expression VisitExtension(Expression extensionExpression)
    {
        switch (extensionExpression)
        {
            case GroupByShaperExpression groupByShaperExpression:
                ...
                return ((FileBaseContextQueryExpression)groupShapedQueryExpression.QueryExpression)
                    .Clone(groupShapedQueryExpression.ShaperExpression);

            case ShapedQueryExpression shapedQueryExpression:
                return ((FileBaseContextQueryExpression)shapedQueryExpression.QueryExpression)
    ...
    {
        var queryExpression = new FileBaseContextQueryExpression(entityType);
    ...
    {
        var inMemoryQueryExpression1 = (FileBaseContextQueryExpression)source1.QueryExpression;
        var inMemoryQueryExpression2 = (FileBaseContextQueryExpression)source2.QueryExpression;
    ...
    private Expression ExpandSharedTypeEntities(FileBaseContextQueryExpression queryExpression, Expression lambdaBody)
        ...
        LambdaExpression innerKeySelector)
    {
        ...
        outerKeySelector = Expression.Lambda(outerKeyBody, ((FileBaseContextQueryExpression)outer.QueryExpression).CurrentParameter);
        innerKeySelector = Expression.Lambda(innerKeyBody, ((FileBaseContextQueryExpression)inner.QueryExpression).CurrentParameter);
    ...
    private Expression RemapLambdaBody(ShapedQueryExpression shapedQueryExpression, LambdaExpression lambdaExpression)
    {
        ...
        return ExpandSharedTypeEntities((FileBaseContextQueryExpression)shapedQueryExpression.QueryExpression, lambdaBody);
    }
        ...
        bool preserveType = false)
    {
        ...
        return lambdaBody != null
            ? Expression.Lambda(
                lambdaBody,
                ((FileBaseContextQueryExpression)shapedQueryExpression.QueryExpression).CurrentParameter)
            : null;
    }
    ...
    private sealed class SharedTypeEntityExpandingExpressionVisitor : ExpressionVisitor
    {
        ...
        private FileBaseContextQueryExpression _queryExpression;

        public SharedTypeEntityExpandingExpressionVisitor(FileBaseContextExpressionTranslatingExpressionVisitor expressionTranslator)
        {
        ...
        public Expression Expand(FileBaseContextQueryExpression queryExpression, Expression lambdaBody)
        {
        ...
        private Expression TryExpand(Expression source, MemberIdentity member)
        {
            ...
            if (navigation.IsCollection)
            {
                ...
                var innerQueryExpression = (FileBaseContextQueryExpression)innerShapedQuery.QueryExpression;
