...
public class FileBaseContextExpressionTranslatingExpressionVisitor : ExpressionVisitor
{
    ...
    private static readonly MethodInfo ParameterValueExtractorMethod =
        typeof(FileBaseContextExpressionTranslatingExpressionVisitor).GetTypeInfo().GetDeclaredMethod(nameof(ParameterValueExtractor))!;

    private static readonly MethodInfo ParameterListValueExtractorMethod =
        typeof(FileBaseContextExpressionTranslatingExpressionVisitor).GetTypeInfo().GetDeclaredMethod(nameof(ParameterListValueExtractor))!;

    private static readonly MethodInfo GetParameterValueMethodInfo =
        typeof(FileBaseContextExpressionTranslatingExpressionVisitor).GetTypeInfo().GetDeclaredMethod(nameof(GetParameterValue))!;
    ...
    private static readonly MethodInfo InMemoryLikeMethodInfo =
        typeof(FileBaseContextExpressionTranslatingExpressionVisitor).GetTypeInfo().GetDeclaredMethod(nameof(InMemoryLike))!;
    ...
    public FileBaseContextExpressionTranslatingExpressionVisitor(
    ...
    protected override Expression VisitMethodCall(MethodCallExpression methodCallExpression)
    {
        ...
        {
            var subquery = (FileBaseContextQueryExpression)subqueryTranslation.QueryExpression;
    ...
    private static Expression ProcessSingleResultScalar(
        FileBaseContextQueryExpression FileBaseContextQueryExpression,
        Expression readValueExpression,
        Type type)
    {
        if (FileBaseContextQueryExpression.ServerQueryExpression is not NewExpression)
        {
            ...
            FileBaseContextQueryExpression.ReplaceProjection(
                new Dictionary<ProjectionMember, Expression> { { new ProjectionMember(), readValueExpression } });
            FileBaseContextQueryExpression.ApplyProjection();
        }

        var serverQuery = FileBaseContextQueryExpression.ServerQueryExpression;
